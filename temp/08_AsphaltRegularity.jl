...
function run_real_experiment(dataset_name, dataset_path)
    ...
    for seed_idx in 1:n_seeds
        ...
        # For univariate, also test derivative variant
        if !is_multivariate_data
            println("  Running WICMAD (deriv)...")
            Y_deriv = derivatives_transform(data.Y_interp, data.t)
            sel_deriv = select_wavelet(Y_deriv, data.t, data.revealed_idx;
                wf_candidates = nothing, J = nothing, boundary = "periodic",
                mcmc = (n_iter=3000, burnin=1000, thin=1), verbose=false)
            wf_deriv = sel_deriv.selected_wf
            
            res_deriv = wicmad(Y_deriv, data.t; n_iter=5000, burn=2000, thin=1,
                              wf=wf_deriv, revealed_idx=data.revealed_idx, verbose=false)
            mapr_deriv = map_from_res(res_deriv)
            metrics_deriv = compute_metrics(mapr_deriv.z_hat, data.gt_binary)
            seed_results["deriv"] = (; metrics=metrics_deriv, res=res_deriv, mapr=mapr_deriv, data=data)
            
            println("    F1 (deriv): $(round(metrics_deriv.f1, digits=4))")
            
            # Find best F1 across all variants
            best_f1_this_seed = max(metrics_raw.f1, metrics_deriv.f1)
        else
    ...
    # For univariate, display derivative variant
    if !is_multivariate_data && haskey(best_results, "deriv")
        metrics_deriv = best_results["deriv"].metrics
        print_metrics("WICMAD (deriv) - $dataset_name [Best Seed: $best_seed]", metrics_deriv)
        push!(csv_rows, (;
            method = "WICMAD",
            variant = "deriv",
            precision = metrics_deriv.precision,
            recall = metrics_deriv.recall,
            f1 = metrics_deriv.f1,
            accuracy = metrics_deriv.accuracy,
            tp = metrics_deriv.tp,
            tn = metrics_deriv.tn,
            fp = metrics_deriv.fp,
            fn = metrics_deriv.fn
