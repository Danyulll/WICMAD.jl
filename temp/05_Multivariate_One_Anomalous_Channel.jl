...
function compute_fdepth_fmp(Y_list, t, y_true)
    try
        ...
        if (n_time != length($t)) {
            stop(paste("Time dimension mismatch: array has", n_time, "time points but t has", length($t)))
        }
        ...
        for (j in seq_len(n_channels)) {
            ...
            # Verify dimensions
            if (nrow(channel_data) != n_obs || ncol(channel_data) != n_time) {
                stop(paste("Channel", j, "dimensions incorrect"))
            }
