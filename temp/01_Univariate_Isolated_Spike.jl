...
# WICMAD: Univariate Isolated Spike Experiment
# This script runs WICMAD on the Univariate Isolated Spike simulated dataset.
...
# ============================================================================
# Experiment: Univariate Isolated Spike
# ============================================================================
...
function run_simulated_experiment(anomaly_type, dataset_name; is_multivariate=false, regime=nothing)
    ...
    for mc_idx in 1:mc_trials
        ...
        # Wavelet selection for raw data
        sel_raw = select_wavelet(data_mc.Y_list, data_mc.t, revealed_idx;
            wf_candidates = nothing, J = nothing, boundary = "periodic",
            mcmc = (n_iter=3000, burnin=1000, thin=1), verbose=false)
        wf_raw = sel_raw.selected_wf
        
        res = wicmad(data_mc.Y_list, data_mc.t; n_iter=5000, burn=2000, thin=1,
                     wf=wf_raw, revealed_idx=revealed_idx, verbose=false)
        ...
        sel_deriv = select_wavelet(Y_deriv, data_mc.t, revealed_idx;
            wf_candidates = nothing, J = nothing, boundary = "periodic",
            mcmc = (n_iter=3000, burnin=1000, thin=1), verbose=false)
        wf_deriv = sel_deriv.selected_wf
        
        # WICMAD automatically handles multivariate data; revealed_idx still valid (observation indices)
        res_deriv = wicmad(Y_deriv, data_mc.t; n_iter=5000, burn=2000, thin=1,
                          wf=wf_deriv, revealed_idx=revealed_idx, verbose=false)
        mapr_deriv = map_from_res(res_deriv)
        wicmad_metrics_deriv = compute_metrics(mapr_deriv.z_hat, data_mc.y_true)
        push!(all_metrics_wicmad_deriv, wicmad_metrics_deriv)
        # Store WICMAD derivative-augmented per-trial result for CSV
        push!(csv_rows, (;
            mc_trial = mc_idx,
            method = "WICMAD",
            variant = "deriv",
            precision = wicmad_metrics_deriv.precision,
            recall = wicmad_metrics_deriv.recall,
            f1 = wicmad_metrics_deriv.f1,
            accuracy = wicmad_metrics_deriv.accuracy,
            tp = wicmad_metrics_deriv.tp,
            tn = wicmad_metrics_deriv.tn,
            fp = wicmad_metrics_deriv.fp,
            fn = wicmad_metrics_deriv.fn
        ))
    end
    ...
    if !isempty(all_metrics_wicmad_deriv)
        median_metrics_wicmad_deriv = (
            ...
            f1 = median([m.f1 for m in all_metrics_wicmad_deriv]),
            accuracy = median([m.accuracy for m in all_metrics_wicmad_deriv]),
            tp = round(Int, median([m.tp for m in all_metrics_wicmad_deriv])),
            tn = round(Int, median([m.tn for m in all_metrics_wicmad_deriv])),
