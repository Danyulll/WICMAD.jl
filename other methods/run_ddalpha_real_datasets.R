...
cat("=== ddalpha Analysis Script (Real Datasets) ===\n")
...
process_real_dataset <- function(csv_file, depth_type = "BD", verbose = TRUE) {
  ...
  tryCatch({
    ...
    if (verbose) {
      cat(" depth computed, finding threshold that maximizes F1...")
    }
    
    # Find threshold that maximizes F1 score
    best_result <- find_best_f1_threshold(anomaly_scores, true_labels)
...
process_all_real_datasets <- function(dataset_folder, depth_type = "BD", verbose = TRUE) {
  ...
  for (csv_file in real_files) {
    result <- process_real_dataset(csv_file, depth_type = depth_type, verbose = verbose)
    if (!is.null(result)) {
...
print_summary <- function(results) {
  ...
  cat("ddalpha RESULTS SUMMARY (Real Datasets)\n")
  cat(paste(rep("=", 100), collapse=""), "\n")
...
save_results_to_csv <- function(results, output_file = "ddalpha_real_results.csv") {
  rows <- list()
  for (dataset_name in names(results)) {
    m <- results[[dataset_name]]$metrics
...
results <- tryCatch({
  process_all_real_datasets(dataset_folder, depth_type = depth_type, verbose = TRUE)
}, error = function(e) {
