...
println("Loading FingerMovements dataset (combining TRAIN + TEST)...")
ds_loaded = load_ucr_dataset("FingerMovements/FingerMovements", data_dir)
...
# Interpolate each multivariate series to a power-of-two length
function interpolate_to_length(series::Vector{Matrix{Float64}}; target_len::Int)
...
# Force target length to 16
targetP = 16
Y, P = interpolate_to_length(prepped.series; target_len=targetP)
M = size(Y[1], 2)
t = collect(1:P)
...
plots_dir = joinpath(dirname(@__DIR__), "plots", "fingermovements")
mkpath(plots_dir)
