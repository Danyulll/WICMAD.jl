...
function load_plane_data(filepath)
...
# Load train+test
train_path = joinpath(@__DIR__, "../data/Plane/Plane_TRAIN.txt")
test_path  = joinpath(@__DIR__, "../data/Plane/Plane_TEST.txt")
Xtr, ytr = load_plane_data(train_path)
Xte, yte = load_plane_data(test_path)
X = vcat(Xtr, Xte)
y = vcat(ytr, yte)

println("Loaded Plane: N=$(size(X,1)), P=$(size(X,2)), classes=$(sort(unique(y)))")
...
plots_dir = joinpath(@__DIR__, "../plots/init_exp/plane")
