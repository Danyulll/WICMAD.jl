...
Xtr, ytr = load_arrowhead_data(train_path)
Xte, yte = load_arrowhead_data(test_path)
X = vcat(Xtr, Xte)
y = vcat(ytr, yte)

println("Loaded ArrowHead: N=$(size(X,1)), P=$(size(X,2)), classes=$(sort(unique(y)))")
...
# Enforce 15% anomalies by subsetting (normal = class 0, anomaly = class 2)
normal_class = 0
anomaly_class = 2
...
total_samples_full = length(y)
n_anomaly_target = round(Int, 0.15 * total_samples_full)
n_anomaly_used = min(length(anomaly_indices_full), n_anomaly_target)
n_normal_used = min(length(normal_indices_full), total_samples_full - n_anomaly_used)
...
# Build inputs for WICMAD on subset
Y = [X[i, :] for i in 1:size(X,1)]
t = collect(1:size(X,2))

# Ensure plots directory exists
plots_dir = joinpath(@__DIR__, "../plots/init_exp")
isdir(plots_dir) || mkpath(plots_dir)

# Wavelet selection using 50% of normals as revealed indices (subset)
normal_indices = findall(==(normal_class), y)
n_revealed = max(1, round(Int, 0.9 * length(normal_indices)))
revealed_subset = sample(normal_indices, n_revealed, replace=false)
revealed_idx = sort(revealed_subset)
println("Revealing $(length(revealed_idx)) of $(length(normal_indices)) normals for wavelet selection (90%)")
...
# Binary ground-truth labels for anomaly evaluation (normal = class 0)
...
# Plots before and after (raw, channel 1)
...
# Dahl and F1-max partitions (from MCMC samples)
...
Y_d1_mats = Y_d1  # already matrices
