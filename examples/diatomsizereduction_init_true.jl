...
function load_diatom_data(filepath)
...
# Load train+test
train_path = joinpath(@__DIR__, "../data/DiatomSizeReduction/DiatomSizeReduction_TRAIN.txt")
test_path  = joinpath(@__DIR__, "../data/DiatomSizeReduction/DiatomSizeReduction_TEST.txt")
Xtr, ytr = load_diatom_data(train_path)
Xte, yte = load_diatom_data(test_path)
X = vcat(Xtr, Xte)
y = vcat(ytr, yte)

println("Loaded DiatomSizeReduction: N=$(size(X,1)), P=$(size(X,2)), classes=$(sort(unique(y)))")
...
"""
Choose majority class as normal, and a single other class as anomaly.
Then subset to achieve ~15% anomalies while keeping normals the majority.
"""
cm = countmap(y)
...
plots_dir = joinpath(@__DIR__, "../plots/init_exp/diatomsizereduction")
