...
function load_strawberry_data(filepath)
...
# Load train+test
train_path = joinpath(@__DIR__, "../data/Strawberry/Strawberry_TRAIN.txt")
test_path  = joinpath(@__DIR__, "../data/Strawberry/Strawberry_TEST.txt")
Xtr, ytr = load_strawberry_data(train_path)
Xte, yte = load_strawberry_data(test_path)
X = vcat(Xtr, Xte)
y = vcat(ytr, yte)

println("Loaded Strawberry: N=$(size(X,1)), P=$(size(X,2)), classes=$(sort(unique(y)))")
...
plots_dir = joinpath(@__DIR__, "../plots/init_exp/strawberry")
