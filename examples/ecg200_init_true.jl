...
function load_ecg200_data(filepath)
...
# Load train+test
train_path = joinpath(@__DIR__, "../data/ECG200/ECG200_TRAIN.txt")
test_path  = joinpath(@__DIR__, "../data/ECG200/ECG200_TEST.txt")
Xtr, ytr = load_ecg200_data(train_path)
Xte, yte = load_ecg200_data(test_path)
X = vcat(Xtr, Xte)
y = vcat(ytr, yte)

println("Loaded ECG200: N=$(size(X,1)), P=$(size(X,2)), classes=$(sort(unique(y)))")
...
plots_dir = joinpath(@__DIR__, "../plots/init_exp/ecg200")
