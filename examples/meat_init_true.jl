...
function load_meat_data(filepath)
...
# Load train+test
train_path = joinpath(@__DIR__, "../data/Meat/Meat_TRAIN.txt")
test_path  = joinpath(@__DIR__, "../data/Meat/Meat_TEST.txt")
Xtr, ytr = load_meat_data(train_path)
Xte, yte = load_meat_data(test_path)
X = vcat(Xtr, Xte)
y = vcat(ytr, yte)

println("Loaded Meat: N=$(size(X,1)), P=$(size(X,2)), classes=$(sort(unique(y)))")
